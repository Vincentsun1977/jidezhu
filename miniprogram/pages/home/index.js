...
Page({
  ...
  async onRecordEnd() {
    ...
    try {
      ...
      const userId = (getApp().globalData.userProfile && getApp().globalData.userProfile.userId) || "u_mock_001";
      ...
      const looksLikeRecallQuestion =
        /我上次|我之前|我以前|我有没有|我是不是|哪个|哪家|哪里|哪天|哪一天|哪一节|哪次|什么|什么时候|几点|多少|吗|呢|呀|来着|帮我找|帮我查|帮我想想/.test(
          intentResult.text || ""
        );
  ...
  async submitTextMemory() {
    ...
    try {
      const result = await createMemoryFromText({
        userId: (getApp().globalData.userProfile && getApp().globalData.userProfile.userId) || "u_mock_001",
  ...
  async ensureReplyAudio(memoryResult) {
    const currentAudioFileId = (memoryResult && memoryResult.replyAudioFileId) || this.data.replyAudioFileId;
    const currentRequestId = (memoryResult && memoryResult.replyRequestId) || this.data.replyRequestId;
    const userId =
      (getApp().globalData.userProfile && getApp().globalData.userProfile.userId) || "u_mock_001";
    ...
    if (result && result.success && result.data && result.data.audioFileId) {
