...
Page({
  data: {
    summaryText: "",
    items: [] as Array<{ id: string; summary: string; timeText: string }>,
  ...
  async onLoad(options: { preset?: string; query?: string }) {
  ...
  async runPresetQuery(preset: string) {
  ...
  async handlePresetTap(
    ...
    await this.runPresetQuery(event.currentTarget.dataset.preset);
  },
  ...
  async runQueryText(query: string) {
    this.setData({
      loading: true,
      errorMessage: "",
    });
